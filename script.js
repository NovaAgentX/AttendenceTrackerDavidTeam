...
function renderVacationsTable() {
  ...
  tbody.innerHTML = AppState.vacations.map((vac, idx) => {
    ...
    const leaveLabel = lopDays > 0 ? `${leaveType}+${lopDays}LOP` : leaveType;
...
function updateVacationSummaryPreview() {
  ...
  let html = `📅 <strong>Total: ${total} days</strong> &nbsp;|&nbsp; 🏖️ VIL: <strong>${vil} days</strong>`;
  if (lop > 0) html += ` &nbsp;|&nbsp; ⚠️ LOP included: <strong>${lop} days</strong>`;
  if (returnDate) html += ` &nbsp;|&nbsp; 🔙 Returns: <strong>${formatDateForDisplay(returnDate)}</strong>`;
...
window.openAddVacationModal = function() {
  ...
  ['vacationStartInput','vacationEndInput','vacationLopDaysInput','vacationReturnDateInput'].forEach(id => {
